...
app.post("/signup", async (req, res) => {
  ...
  try {
    // using container name works for docker but not in k8s
    // use an env var to externalise
    const hashedPW = await axios.get(
      `http://${process.env.AUTH_ADD}/hashed-password/` + password
    );
    // const hashedPW = "dummy password for time being";
...
app.post("/login", async (req, res) => {
  ...
  // temp hack until we bring up all containers
